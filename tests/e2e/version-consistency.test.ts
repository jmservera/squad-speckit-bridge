 ...
 * status --json, CLI --version) report the same version matching package.json.
 */
...
import { createInstaller, createStatusChecker } from '../../src/main.js';
...
describe('version consistency across CLI surfaces', () => {
  ...
  it('CLI --version outputs the correct version', () => {
  ...
  it('all surfaces report the identical version', async () => {
    ...
    const cliOutput = execSync('node dist/cli/index.js --version', {
      encoding: 'utf-8',
      cwd: join(import.meta.dirname!, '../..'),
    }).trim();
