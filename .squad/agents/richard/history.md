...
### 2026-03-23 — Squad v0 Meta-Analysis (Full Pipeline Retrospective)

- **Parallel fan-out research is the highest-leverage Squad pattern.** Four agents (Gilfoyle, Dinesh, Jared, Richard) completing independent analysis in ~15 minutes produced dramatically better spec input than sequential exploration. This is the pattern to preserve and strengthen in v1.
- **50-task decomposition is over-engineered for AI agents.** Feature 003 generated 50 tasks (T001–T050) mapped to 50 GitHub issues. The implementation phase closed 17 issues in 25 minutes — each averaging 90 seconds of real work. Tasks like "add execution timing tracking" (T011) and "generate ExecutionReport" (T012) should have been a single task. Target 15-20 tasks per feature.
- **Agent distribution is severely skewed.** Dinesh held 193 of 210 squad-labeled issues (92%). Monica held 16. Gilfoyle and Jared held zero implementation issues. Ralph's routing function needs rebalancing rules — no single agent should exceed 50% of implementation work.
- **The bridge didn't eat its own dog food.** Issue creation used a generated shell script (`create-issues.sh`, 22KB) instead of the bridge's own `issues` command. This bypassed the product we're building and missed an opportunity to validate the UX.
- **Test-last is a process smell.** Tasks.md placed testing in Phase 8, after all implementation. The result: 17 implementation commits landed with zero test coverage for the demo feature. All 183 passing tests are from the bridge core (v0.1/v0.2), not from feature 003. Co-locating tests with implementation tasks prevents this gap.
- **Planning-to-implementation ratio was extreme: ~3 hours planning, 25 minutes implementing.** The planning produced high-quality artifacts (spec.md, plan.md, tasks.md, 5 clarifications, Clean Architecture boundary analysis) but the ratio should converge. Much of the planning overhead was first-run discovery — subsequent features should plan faster.
- **Issue-driven commit workflow is clean and traceable.** Every commit references its issue number (#205–#221), creating a complete audit trail from task → issue → commit → code. This is worth preserving.
- **Label taxonomy needs hierarchy.** Current labels are flat: phase:setup, layer:entity, squad:dinesh, enhancement, testing. A hierarchical scheme (area/, type/, agent/, phase/) would reduce noise in issue lists.
- **Decision proposal written to** `.squad/decisions/inbox/richard-v0-retrospective.md` with P0/P1/P2 prioritized improvements.

### 2025-07-25 — SpecKit Integration Tooling Deep Analysis

- **Bridge adoption is 14%.** Of 7 CLI commands built (`install`, `context`, `status`, `review`, `issues`, `sync`, `demo`), only `install` was used. Zero `squad-context.md` files exist. Zero `review.md` files exist. The `issues` command was bypassed in favor of a 22KB shell script. The bridge is architecturally sound but operationally invisible.
- **Hooks don't fire when agents are used instead of CLI.** The 3 lifecycle hooks (`before-specify.sh`, `after-tasks.sh`, `after-implement.sh`) are deployed and executable but never triggered. Root cause: SpecKit agents (`/speckit.specify`, `/speckit.plan`, `/speckit.tasks`) run in Copilot Chat, which bypasses Spec Kit's extension hook system. The hooks were designed for a CLI workflow; we deployed in an agent workflow.
- **The knowledge flywheel has zero rotations.** The Squad→SpecKit direction (squad-context.md injecting decisions, skills, and learnings into planning) was designed but never activated. Planning cycles start from zero knowledge despite agents accumulating rich learnings in history.md. This is the highest-leverage gap to close.
- **Skills exist but aren't consumed.** `speckit-bridge/SKILL.md` (186 lines) and `clean-architecture-bridge/SKILL.md` (244 lines) contain excellent compressed knowledge. No evidence they were passed to agent prompts during orchestration. `project-conventions/SKILL.md` remains an unfilled template.
