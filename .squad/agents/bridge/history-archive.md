...
- **Role:** Lead Orchestrator for squad-speckit-bridge project
- **Domain:** Build integration layer between Squad (multi-agent orchestration) and Spec Kit (specification pipeline)
- **Key Files:** Hook scripts (`before-specify.sh`, `after-implement.sh`, `after-tasks.sh`), config schema, demo command
- **Critical Learnings:** 
  - **Hook Architecture:** Executable bits must be set via `git update-index --chmod=+x` (filesystem + git tracking). After-tasks hook automates issue creation with Squad label.
  - **CLI Naming:** `squask` is primary alias, fully replaces `npx squad-speckit-bridge` in hook scripts for consistency.
  - **Config Design:** `autoCreateIssues` boolean in `BridgeConfig.hooks` controls automation; defaults gracefully if config missing.
  - **Demo Command:** Exports ExecutionReport with success/failure variants; cleans up artifacts on success by default; JSON output schema fully documented.
  - **Clean Architecture:** All changes must preserve entity/adapter/port separation; composition root wires dependencies only.
  - **Testing Pattern:** File-based hook tests verify content + automation behavior; permission tests use `deployExecutable()` with `0o755` validation.
- **Status:** v0.3.1 spike complete; sync flywheel validated; histories being compacted.
